...
describe('AvatarFace Unit Test', () => {
  ...
  describe('Token URI', () => {
    ...
    it('Should return a token URI', async () => {
      const tokenURI = await avatarFace.tokenURI(0)
  ...
  describe('Token SVG', () => {
    ...
    it('Should return SVG data for the token', async () => {
      const tokenSVG = await avatarFace.tokenSVG(0)
    ...
    it('Should revert if the token does not exist', () => {
      expect(avatarFace.tokenSVG(1)).to.be.revertedWith('AvatarFace: Token does not exist')
    })
  })
})